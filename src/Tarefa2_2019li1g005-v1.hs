module Tarefa2_2019li1g005 where
...
sobe m player | diffh < (-0.2) = player{pistaJogador = y - 1,estadoJogador = Ar {alturaJogador = h1, inclinacaoJogador = 0 + ((180*inclin peca1)/pi), gravidadeJogador = 0}} 
...
desce m player | diffh < (-0.2) = player{pistaJogador = y + 1,estadoJogador = Ar {alturaJogador = h1, inclinacaoJogador = 0 + ((180*inclin peca1)/pi), gravidadeJogador = 0}} 
               | diffh < 0.2 = player {pistaJogador = y + 1}  
               | otherwise = player {estadoJogador = Morto {timeoutJogador=1},velocidadeJogador = 0}
               where (x,y) = (distanciaJogador player,pistaJogador player)
                     peca1 = encontraPosicaoMatriz (y,floor x) m
...
inclinaE player | (normalizaAngulo (ang + 15)) >= 90 = player {estadoJogador = estado {inclinacaoJogador = 90}}
                | otherwise = player {estadoJogador = estado {inclinacaoJogador = ang + 15}}
...
inclinaD player | (ang - 15) <= (-90) = player {estadoJogador = estado {inclinacaoJogador = (-90)}}
                | otherwise = player {estadoJogador = estado {inclinacaoJogador = ang - 15}}
                where estado = estadoJogador player
                      ang = inclinacaoJogador estado
