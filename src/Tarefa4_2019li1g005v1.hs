...
module Tarefa4_2019li1g005v1  where
...
acelera t m j | (estaChao estPlayer) = let accelMota = if v < 2 then 1.0 else 0.0
                                           ...
                                           atrito = if ePosicaoMatrizValida (x,y) m then pecaAtrito ((m!!x)!!y) else pecaAtrito (Recta Terra 0)
...
move t m j | estaMorto estPlayer = let tm = timeoutJogador estPlayer 
           ...
           where estPlayer = estadoJogador j
                 ...
                 pista = (m!!y)
                 peca = if eIndiceListaValido x pista then pista!!x else (Recta Terra 0)
                 peca1 =if eIndiceListaValido (x+1) pista then pista!!(x+1) else (Recta Terra 0) 

whenFloor :: Peca -> Peca -> Double -> Jogador -> Jogador
whenFloor peca peca1 t j| (d + distr) > lim = if incl > incl1 then (j{distanciaJogador = lim,estadoJogador = Ar (alt peca) (incl*180/pi) 0 (incl*180/pi)})
...
whenAr peca t j |intersetam reta1 reta2 = let incl = (inclin peca)*(pi)/180 
                ...
                where estPlayer = estadoJogador j
                      ...
                      vel = Polar (velocidadeJogador j) (inclinacaoJogadorini estPlayer)
                      ...
                      reta2 = ((Cartesiano (d-0.5) ((alti peca))),(Cartesiano (d+1) (alt peca)))
